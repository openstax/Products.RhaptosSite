...
# Overrides CMFPlone/skins/plone_scripts/livesearch_reply.py
...
# Instead of the portal_catalog use content.catalog
...
# TODO: Until we have a SearchableText field on the content catalog, use the Title
# results = catalog(SearchableText=r, portal_type=friendly_types)
results = catalog(Title=r, portal_type=friendly_types)
...
if not results:
    ...
    # Send the user to "search" instead of the default "search_form"
    write('<a href="search" style="font-weight:normal">%s</a>' % ts.translate(label_advanced_search))
    write('''</div>''')
    write('''</div>''')
    write('''</fieldset>''')
    ...
    for result in results[:limit]:
        ...
        # Don't attach /view to search results
        ...
        # TODO: Concatenating titles causes an error so don't concatenate yet
        ...
        # TODO: Concatenating descriptions causes an error so don't concatenate yet
